...
def text2excel():
    ...
    #背景色
    # blankBkg = xlwt.easyxf('pattern: pattern solid, fore_colour gold; font: bold on;borders: left 1, right 1, top 1, bottom 1;')
    note_style = xlwt.easyxf('font: height 400, name SimSun, colour_index red, bold on; align: wrap on, vert centre, horiz center;borders: left 1, right 1, top 1, bottom 1;' )
    text_style = xlwt.easyxf('font: height 250, name SimSun, colour_index black; align: wrap on, vert centre, horiz center;borders: left 1, right 1, top 1, bottom 1;' )
    text1_style = xlwt.easyxf('font: height 250, name SimSun, colour_index black; align: wrap on, vert centre, horiz center;pattern: pattern solid;borders: left 1, right 1, top 1, bottom 1;' )
    text2_style = xlwt.easyxf('font: height 250, name SimSun, colour_index black; align: wrap on, vert centre, horiz center;pattern: pattern solid;borders: left 1, right 1, top 1, bottom 1;' )
    ...
    for idx,q in enumerate(json_result(input_file)):
        ...
        xlsheet1.write_merge(0, 0, 0, 6, '说明：专家列表，每一个关键字匹配是个最优结果，标注时若与搜索结果匹配则为1，否则为0',note_style)
        xlsheet2.write_merge(0, 0, 0, 4, '说明：专利列表，每一个关键字匹配是个最优结果，标注时若与搜索结果匹配则为1，否则为0',note_style)
  
        xlsheet1.write_merge(11*idx+2, 11*idx+11, 0, 0, query[idx].rstrip(),text_style)
        xlsheet2.write_merge(11*idx+2, 11*idx+11, 0, 0, query[idx].rstrip(),text_style)
        xlsheet1.write_merge(11*idx+12, 11*idx+12, 0, 6,'')
        xlsheet2.write_merge(11*idx+12, 11*idx+12, 0, 4,'')
        ...
        for i in range(headlen1):
            xlsheet1.write(1,i,teachers_head[i],text_style)
        #写入表内容
        ...
        for row in range(contentRow1):
            for col in range(headlen1-2):
                if row%2==0:
                    xlsheet1.write(11*idx+row+2,col+1,content1[row][0][col],text1_style)
                else:
                    xlsheet1.write(11*idx+row+2,col+1,content1[row][0][col],text2_style)
            
            xlsheet1.col(4).width = 256*36
            xlsheet1.col(5).width = 256*100
            for k in range(4):
        ...
        for i in range(headlen2):
            xlsheet2.write(1,i,patents_head[i],text_style)
        #写入表内容
        ...
        for row in range(contentRow2):
            for col in range(headlen2-2):
                if row%2==0:
                    xlsheet2.write(11*idx+row+2,col+1,content2[row][0][col],text1_style)
                else:
                    xlsheet2.write(11*idx+row+2,col+1,content2[row][0][col],text2_style)
            xlsheet2.col(2).width = 256*36
            xlsheet2.col(3).width = 256*100
            for k in range(2):
                xlsheet2.col(k).width = 256*12
            
    workbook.save(r'F:\\projects\\match_gz\\test\\querytxt.xls')
